...
def main(config_path, signal_length):
    ...
    save_path = f'results/accuracy_w_input_noise/'
    os.makedirs(save_path, exist_ok=True)

    print('sigma_in accuracy')
    # performanceは1つの学習済みモデルに対してsigma_neu^testを0から0.1まで変えてそれぞれの正解率を記録する。
    results_acc = np.zeros(5)
    sigma_in_list = [0, 0.05, 0.1, 0.15, 0.2]
    ...
    for acc_idx in range(5):
        ...
        sigma_in = sigma_in_list[acc_idx]

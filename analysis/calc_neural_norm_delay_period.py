...
def main(config_path, sigma_in):
    ...
    for delta_idx in range(50):
        ...
        for i in range(10):
            ...
            results_norm.append(np.linalg.norm(hidden_list.cpu().detach().numpy()[:, 50: 150, :]))
        num_data += 500
        if delta > 0:
            ans = 1
        else:
    ...
    np.savetxt(os.path.join(save_path, f'{model_name}_delayperiod.txt'), np.array([np.mean(results_norm), np.std(results_norm)]))

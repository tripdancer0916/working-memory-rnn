...
def romo_signal(batch_size, signal_length, sigma_in, time_length=400, alpha=0.25):
    ...
    for i in range(batch_size):
        ...
        first_signal = np.sin(omega_1 * t) + np.random.normal(0, sigma_in, signal_length)
...
def main(config_path):
    ...
    input_signal, omega_1_list = romo_signal(trial_num, signal_length=15, sigma_in=0.0, time_length=1000)
    ...
    plt.savefig(f'results/{model_name}/coding_angle_Ts2.png', dpi=200)
    ...
    plt.savefig(f'results/{model_name}/coding_angle_Tf2.png', dpi=200)
    # np.save(f'results/{model_name}/active_norm.npy', np.array(angle_list))
